...
func (b *BiQuGeCrawler) FetchChapterList() ([]Chapter, error) {
	...
	// 获取章节目录信息
	r := make([]Chapter, 0)
	dom.Find(my_global.BiQuGeInfoByHost[b.novelUrl.Hostname()].ASelector).Each(func(i int, selection *goquery.Selection) {
		// 获取a标签链接
...
func (b *BiQuGeCrawler) FetchChapterContent(c *Chapter) error {
	...
	// 获取章节content
	c.Content, err = dom.Find(my_global.BiQuGeInfoByHost[b.novelUrl.Hostname()].ContentSelector).Html()
	if err != nil {
		return err
	}
	...
	// 删除content文本中的某些标签
	for _, v := range my_global.BiQuGeInfoByHost[b.novelUrl.Hostname()].RemoveSelector {
	...
	// 对content字符串进行替换
	rp := my_global.BiQuGeInfoByHost[b.novelUrl.Hostname()].StrReplace
	for k, v := range rp {
